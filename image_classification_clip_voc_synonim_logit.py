...
def read_synonyms(file_name):
    #
    # Reads list of synonims. Produces a list of synonims for the 20 VOC classes, plus background (maybe)
    #
...
def generate_query_synonym(query_names, addplurals=False):
    ...
    for i, query in enumerate(query_names):
        ...
        for name in query:
            labels.append(i)
            prompts.append(clip.tokenize(f"A photo of a %s" % name))
            if addplurals:
                labels.append(i)
                prompts.append(clip.tokenize(f"A photo of a %ss" % name))
...
def multi_label_synonym(image_features, text_features, class_names, labels, tau=100.0):
    #
    #  Gets logits using the best synonim for each category
    #

    #
    #  query_names contain the prompt embeddings, and the corresponding class labels are in varaible labels
    #
...
if __name__ == '__main__':
    ...
    #
    image_dir = 'D:\\Dados\\' + dataset + '\\JPEGImages\\'
    class_txt = 'D:\\Dados\\' + dataset + '\\voc_class_names.txt'
    ann_dir = 'D:\\Dados\\' + dataset + '\\Annotations\\'
    # Directory with the results of TAI-DPT (Resnet50)
    dai_dir = "D:\\OneDrive\\My Python Files\\clip mining\\tai_dpt\\voc_out\\voc_out_test\\"
    ...
    for modelname in [available_models[6]]:  # batch processing of different models
        ...
        if LoadFile:
            ...
            all_det_labels_synonym, all_gt_labels = pickle.load(file)
            file.close()

            file = open(savedir + '\\' + modelname_pruned +
                        dataset + '_logits_canonical.dat', 'rb')
            all_det_labels_canonical, all_gt_labels = pickle.load(file)
            file.close()
            ...
            #
            #  Gets temperature trained with model (see https://github.com/openai/CLIP/blob/main/clip/model.py)
            #
            ...
            for ind, file in enumerate(tqdm(filelist)):
                ...
                #
                #  Gets logits (inner product between image and text embeddings scaled by temperature)
                #
                syn_logits = multi_label_synonym(
                    image_features, text_features_synonym, class_names, labels, tau)
...
# image_labels = gts = np.load(dai_dir + 'label_%06d.npy'%image_id)
            ...
            if SaveFile:
                ...
                pickle.dump([all_det_labels_synonym, all_gt_labels], file)
                file.close()

                file = open(savedir + '\\' + modelname_pruned +
                            dataset + '_logits_canonical.dat', 'wb')
                pickle.dump([all_det_labels_canonical, all_gt_labels], file)
                file.close()
